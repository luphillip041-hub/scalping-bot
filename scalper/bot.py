...
signals, submits bracket orders for equities AND options (calls/puts),
...
from .options import OptionSide, OptionSelector, OptionContract
...
class ScalpingBot:
    ...
    def _open_positions(self):
        """Fetch all open positions from Alpaca (equities only, not options)."""
        for attempt in range(MAX_RETRIES):
            try:
    ...
    def _submit_option_order(self, symbol: str, side: Side, underlying_price: float):
        ...
        Attempts to fetch the options chain, select a contract, and submit a market order.
        """
        if not self.cfg.enable_options:
            return
        
        try:
            # Fetch options chain for the underlying
            # NOTE: Alpaca's options API requires specific implementation
            # For now, this is a placeholder showing the flow
            option_contract = None
            
            if side == Side.LONG:
                # For LONG signals, buy a call
                log.debug("Fetching call options for %s", symbol)
                # In production, fetch chain and select call via option_selector
                # option_contract = self.option_selector.select_call(underlying_price, chain_contracts)
            else:
                # For SHORT signals, buy a put
                log.debug("Fetching put options for %s", symbol)
                # option_contract = self.option_selector.select_put(underlying_price, chain_contracts)
            
            if not option_contract:
                log.warning("No suitable option contract found for %s", symbol)
                return
            
            # Calculate contracts needed (100 shares per contract)
            contract_notional = option_contract.mid_price * 100
            contracts = max(1, int(self.cfg.options_position_size_usd / contract_notional))
            
            # Submit market order for the contract
            order = MarketOrderRequest(
                symbol=option_contract.contract_id,
                qty=contracts,
                side=OrderSide.BUY,
                time_in_force=TimeInForce.DAY,
                # NOTE: Take profit/stop loss for options would be based on price
                # or automatic close at profit/loss target
            )
            
            placed_order = self.trading.submit_order(order)
            position_key = f"OPT_{symbol}_{option_contract.contract_id}"
            ...
            log.info("ENTER OPTION %s %s x%d @ %.2f (delta %.2f)",
                     side.value, option_contract.contract_id, contracts, 
                     option_contract.mid_price, option_contract.delta)
            send(
                f"📊 Option {side.value.upper()} {symbol}",
                f"{option_contract.side.value.upper()} contract",
                "entry_long" if side == Side.LONG else "entry_short",
                {
                    "Contract": option_contract.contract_id,
                    "Strike": f"${option_contract.strike:.2f}",
                    "Delta": f"{option_contract.delta:.2f}",
                    "Price": f"${option_contract.mid_price:.2f}",
                    "Qty": contracts,
                },
            )
        except Exception as e:
        ...
        for sym in list(self.tracked_positions.keys()):
            ...
            if (now - entered_at) > timedelta(minutes=self.cfg.max_hold_minutes):
                ...
                try:
                    ...
                    qty = 1  # For options, we track by contract, so qty=1
        ...
        for sym in list(self.tracked_positions.keys()):
            ...
            if asset_type == "equity":
                if sym not in current_syms and sym not in self._last_synced_positions:
                    ...
                    entry_price, entered_at, order_id, _ = self.tracked_positions[sym]
                    qty = self.risk.qty_for(entry_price)
            ...
            try:
                ...
                for sym in self.cfg.symbols:
                    ...
                    if sig:
                        ...
                        try:
                            # Submit both equity and option trades on signal
